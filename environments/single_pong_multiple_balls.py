...
class PongEnv(gym.Env):
    ...
    def __init__(self, width=20, height=10, paddle_height=3):
        ...
        self.ball_pos = [self.width // 2 - 1, self.height // 2]
        self.ball2_pos = [self.width // 2 + 1, self.height // 2]

        self.ball_direction = [random.uniform(0.5, 1), random.uniform(0.5, 1)]
        self.ball2_direction = [-1 * random.uniform(0.5, 1), -1 * random.uniform(0.5, 1)]
    ...
    def check_directions(self, ball_pos, ball_direction):
        reward = 0
        ...
        if ball_pos[0] <= 0:
            ball_direction[0] *= -1
        if ball_pos[0] >= self.width - 1 and self.paddle_pos <= ball_pos[1] < self.paddle_pos + self.paddle_height:
            ball_direction[0] *= -1
            ball_pos[0] += ball_direction[0]
            reward = 1
        return ball_pos, ball_direction, reward
    ...
    def step(self, action):
        ...
        # Update ball position and check for collision
        self.ball_pos, self.ball_direction, new_reward = self.check_directions(self.ball_pos, self.ball_direction)
        reward += new_reward

        self.ball2_pos, self.ball2_direction, new_reward = self.check_directions(self.ball2_pos, self.ball2_direction)
        reward += new_reward

        done = False
        if any([self.check_done(self.ball_pos), self.check_done(self.ball2_pos)]):
            done = True
        ...
        rel_ball2_pos = self.get_relative_position(self.ball2_pos)

        obs = np.array([rel_paddle_pos[0], rel_paddle_pos[1], rel_ball_pos[0], rel_ball_pos[1], self.ball_direction[0], self.ball_direction[1], rel_ball2_pos[0], rel_ball2_pos[1], self.ball2_direction[0], self.ball2_direction[1]], dtype=np.float16)
    ...
    def render(self, mode='console'):
        ...
        board[int(self.ball2_pos[1])][int(self.ball2_pos[0])] = 'O'
